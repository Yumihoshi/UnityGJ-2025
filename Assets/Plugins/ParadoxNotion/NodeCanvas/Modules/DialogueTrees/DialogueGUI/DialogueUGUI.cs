...
namespace NodeCanvas.DialogueTrees.UI.Examples
{
    public class DialogueUGUI : MonoBehaviour, IPointerClickHandler
    {
        ...
        public bool keepUIAfterDialogueEnd = false;
        ...
        public void SetOriginalPosition(Vector2 trans)
        {
            originalSubsPosition=trans;
        }
        ...
        {
            // Always stop coroutines and audio
            StopAllCoroutines();
            if (playSource != null) playSource.Stop();

            if (!keepUIAfterDialogueEnd)
            {
                // 原有行为：隐藏 UI 并销毁缓存按钮
                subtitlesGroup.gameObject.SetActive(false);
                optionsGroup.gameObject.SetActive(false);
            ...
            {
                // 保留 UI 可见，但禁用所有按钮以避免交互（你也可以改为保留可交互）
